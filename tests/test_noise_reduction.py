...
def test_spectral_gating(test_tone: AudioData):
    ...
    result = apply_noise_reduction(
        test_tone, noise_threshold_db=-10, attenuation_db=-10, smooth=False
    )
...
def test_wiener_filter(test_tone: AudioData):
    noise_level_db = -20
    test_tone.data += generate_white_noise(
        sample_rate_hz=test_tone.sample_rate,
        duration_s=test_tone.get_duration_s(),
    ...
    result = apply_noise_reduction(
        test_tone, noise_threshold_db=-10, attenuation_db=-10, smooth=True
    )
